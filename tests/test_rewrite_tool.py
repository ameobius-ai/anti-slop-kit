...
class TestValidateRewrite:
    ...
    def test_accept_when_score_same(self):
        ...
        assert result["tool"] == "validate_rewrite"
        assert result["verdict"] == "accept"
        assert result["reasons"] == []
        assert result["score"]["delta_per100w"] == 0
    ...
    def test_reject_when_score_increases(self):
        ...
        assert "score rose" in result["reasons"][0]
        assert result["score"]["delta_per100w"] > 0
    ...
    def test_reject_when_numbers_lost(self):
        ...
        assert result["ok"] is True
        assert result["verdict"] == "reject"
        assert any("lost numbers" in r for r in result["reasons"])
        assert "3.0" in result["fidelity"]["numbers"]
    ...
    def test_reject_when_identifiers_lost(self):
        ...
        assert result["ok"] is True
        assert result["verdict"] == "reject"
        assert any("lost identifiers" in r for r in result["reasons"])
        assert "api_secret" in result["fidelity"]["identifiers"]
    ...
    def test_reject_when_urls_lost(self):
        ...
        assert result["ok"] is True
        assert result["verdict"] == "reject"
        assert any("lost urls" in r for r in result["reasons"])
        assert "https://test.org" in result["fidelity"]["urls"]
...
class TestMain:
    ...
    def test_main_with_invalid_lang(self, tmp_path, capsys):
        ...
        result = rewrite_tool.main(["--lang", "invalid", str(original), str(rewrite)])
        
        assert result == 2
        captured = capsys.readouterr()
        assert "error" in captured.out.lower()
    
    def test_main_with_missing_lang_value(self, tmp_path, capsys):
        ...
        result = rewrite_tool.main(["--lang", str(original), str(rewrite)])
    ...
    def test_main_with_help(self, capsys):
        ...
        result = rewrite_tool.main(["--help"])
        
        assert result == 0
        captured = capsys.readouterr()
        assert "validate_rewrite" in captured.out.lower()
    
    def test_main_with_unknown_flag(self, capsys):
        """Test main() with unknown flag."""
        result = rewrite_tool.main(["--unknown-flag"])
    ...
    def test_main_one_file_only(self, tmp_path, capsys):
        ...
        result = rewrite_tool.main([str(original)])
        
        assert result == 2
        captured = capsys.readouterr()
        assert "error" in captured.out.lower()
    
    def test_main_three_files(self, tmp_path, capsys):
        ...
        result = rewrite_tool.main([str(file1), str(file2), str(file3)])
    ...
    def test_main_nonexistent_file(self, tmp_path, capsys):
        ...
        result = rewrite_tool.main([str(tmp_path / "nonexistent.md"), str(rewrite)])
        
        assert result == 2
        captured = capsys.readouterr()
        assert "error" in captured.out.lower()
    
    def test_main_accept_returns_zero(self, tmp_path):
    ...
    def test_main_reject_returns_one(self, tmp_path):
